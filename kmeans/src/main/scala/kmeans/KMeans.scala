...
class KMeans extends KMeansInterface {
  ...
  def classify(points: Seq[Point], means: Seq[Point]): Map[Point, Seq[Point]] = {
    if (points.isEmpty) {
      means.map(p => p -> Seq()).toMap
    } else {
      points.groupBy(p => findClosest(p, means)) withDefaultValue Seq()
    }
  }

  def classify(points: ParSeq[Point], means: ParSeq[Point]): ParMap[Point, ParSeq[Point]] = {
    if (points.isEmpty) {
      means.map(p => p -> ParSeq()).toMap
    } else {
      points.groupBy(p => findClosest(p, means)) withDefaultValue ParSeq()
    }
  }
  ...
  def quadraticDistance(p: Point, p1: Point): Double = {
    (p.x - p1.x) * (p.x - p1.x) + (p.y - p1.y) * (p.y - p1.y) + (p.z - p1.z) * (p.z - p1.z)
  }

  def update(classified: Map[Point, Seq[Point]], oldMeans: Seq[Point]): Seq[Point] = {
    oldMeans.map(mean =>
      findAverage(mean, classified(mean))
    )
  }
  ...
  def converged(eta: Double, oldMeans: Seq[Point], newMeans: Seq[Point]): Boolean = {
    if (oldMeans.isEmpty) {
      ...
      oldMeans.zip(newMeans).map { case (p, p1) => quadraticDistance(p, p1) }.forall(distance => distance <= eta)
    }
  }
