...
class KMeansSuite {
  ...
  @Test def `converged for Empty`: Unit = {
    assertTrue(converged(0.001, ParVector(), ParVector()))
  }

  @Test def `test`: Unit = {
    //classify(ParVector(), ParVector((1.0, 1.0, 1.0))) should equal to ParHashMap((1.0, 1.0, 1.0) -> ParVector())
    val result = classify(ParVector(), ParVector(new Point(1.0, 1.0, 1.0)))

    assertEquals(ParHashMap(new Point(1.0, 1.0, 1.0) -> ParVector()), result)
  }
}
