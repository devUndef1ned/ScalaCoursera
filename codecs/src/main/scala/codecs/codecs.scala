...
trait DecoderInstances {
  ...
  implicit val intDecoder: Decoder[Int] = Decoder.fromFunction {
    case Json.Num(b) => if (b.isValidInt) Option.apply(b.intValue) else Option.empty[Int]
    case Json.Str(str) => Try(str.toInt).toOption
    case _ => Option.empty
  }

  /** A decoder for `String` values */
  implicit val strDecoder: Decoder[String] = Decoder.fromPartialFunction {
...
//     Uncomment the following lines as you progress in the assignment
    ...
    println(implicitly[Encoder[List[Person]]])
  }
