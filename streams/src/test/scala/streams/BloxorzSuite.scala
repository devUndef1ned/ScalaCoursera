...
class BloxorzSuite {
  trait SolutionChecker extends GameDef with Solver with StringParserTerrain {
    ...
    def solve(ls: List[Move]): Block =
      ls.foldLeft(startBlock) { case (block, move) =>
        println(s"Block is $block apply move $move")
  ...
  @Test def `done for final block`: Unit = new Level1 {
    Assert.assertTrue(done(Block(goal, goal)))
  }

  @Test def `find neighbors with history`: Unit = new Level1 {
  ...
  @Test def `find newNeighborsOnly`: Unit = new Level1 {
