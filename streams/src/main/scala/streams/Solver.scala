...
trait Solver extends GameDef {
  ...
  def from(initial: LazyList[(Block, List[Move])],
    ...
    val more = for {
      ...
      next <- newNeighborsOnly(neighborsWithHistory(blockWithMove._1, blockWithMove._2), explored + blockWithMove._1)
    } yield next
    more match {
      case LazyList() => initial
      case _ => from(more, explored ++ more.map(_._1)) ++ initial
    }
  }
   ...
   */
  lazy val pathsToGoal: LazyList[(Block, List[Move])] = pathsFromStart.filter(blocksWithMove => done(blocksWithMove._1))
  ...
  lazy val solution: List[Move] = pathsToGoal match {
    case LazyList() => List()
    case notEmpty => notEmpty.minBy(_._2.size)._2.reverse
  }
}
